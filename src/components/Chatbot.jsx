...
const Chatbot = ({ forceOpen = false, onClose }) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    // API Call - use environment variable for the API key
    try {
      const API_KEY = import.meta.env.VITE_GEMINI_KEY || '';
      if (!API_KEY) {
        setMessages(prev => [...prev, { role: 'bot', text: "⚠️ Chatbot is not configured (missing API key)." }]);
        setIsThinking(false);
        return;
      }
      ...
      setMessages(prev => [...prev, { role: 'bot', text: botResponseText || "Sorry, I couldn't generate a response." }]);
    } catch (error) {
  ...
  // render
  return (
    <div className={`chatbot ${isOpen || forceOpen ? 'open' : ''}`}>
      <div className={`chatbot-popup ${isOpen || forceOpen ? 'open' : ''} ${forceOpen ? 'embedded-chat' : ''}`}>
